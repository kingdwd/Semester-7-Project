...
%K = -lqrd (Aaugmented,Baugmented,Qespeed,Respeed,Ts)Gspeed
K = -lqr (Aaugmented,Baugmented,Qespeed,Respeed)

%finding the observer gains
%poles = eig(GdSpeed)*0.5
poles = eig(Gspeed)*5
%L = place(GdSpeed.A',GdSpeed.C',poles)
L = place(Gspeed.A',Gspeed.C',[-3+i -3-i])
Ltr = L'
eig(Gspeed.A - Ltr*Gspeed.C)
